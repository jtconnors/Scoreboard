...
This latest version of the source code is tagged ```v1.2-JDK17-maven```.     As its name suggests, it is specific to JDK 15 and can be built with the ```apache maven``` build lifecycle system. Scripts contained in this project will insist that JDK 17 be used, the first LTS release that formally includes the ```jpackage``` utility. Subsequent JDK releases have not been tested with this version of the project.
